...
class simple():
    ...
    @staticmethod
    def case_CAMBIO_DE_GRUPO_PREGRADO(request,docx):
        para = docx.add_paragraph()
        para.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
        para.add_run('El Consejo de Facultad ')
        if request.approval_status == 'AP':
            ...
            para.add_run('NO APRUEBA ').font.bold = True
        para.add_run('cambio de grupo de la asignatura ' + request.detail_cm['subject'] + ', código ' + request.detail_cm['cod'])
        para.add_run(', tipología ' + request.detail_cm['tip'] + ', inscrita en el periodo ' + request.detail_cm['per'])
        para.add_run(', del grupo ' + request.detail_cm['go'] + ' al grupo ' + request.detail_cm['gd'] + ' con el profesor ')
        para.add_run(request.detail_cm['prof'] + ' del departamento de ' + request.detail_cm['dep'])
        if request.approval_status == 'AP':
            para.add_run(', debido a que justifica debidamente la solicitud.')
        else:
            para.add_run(', debido a que ' + request.justification + '.')
