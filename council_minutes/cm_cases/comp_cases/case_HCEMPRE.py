...
class HCEMPRE():
    ...
    def case_HOMOLOGACION_CONVALIDACION_EQUIVALENCIA_PREGRADO_AP(request, docx, paragraph):      
        if request.detail_cm['homologation']:
            ...
            Item = Item1 + Item2
            para = docx.add_paragraph(Item.format( request.academic_period, request.detail_cm['origin_program'], request.detail_cm['origin_institution']), style='List Number')
            para.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
            para.paragraph_format.space_after = Pt(0)
            paragraph.paragraph_format.space_after = Pt(0)
            HCEMPRE.case_HOMOLOGACION_CONVALIDACION_EQUIVALENCIA_PREGRADO_TABLE(request, docx,'homologation')
        elif request.detail_cm['equivalence']:
            Item = 'equivaler'
        elif request.detail_cm['recognition']:
            Item = 'convalidar'
    ...
    def case_HOMOLOGACION_CONVALIDACION_EQUIVALENCIA_PREGRADO_TABLE(request, docx, case):
        ...
        C = 0
        cant_rows = 0
        for i in range (0, len(request.detail_cm[case])):           
        ...
        cellp.alignment = WD_ALIGN_PARAGRAPH.CENTER
        cellp.add_run('Asignaturas cursadas en el plan de estudios {} de la {}'.format(request.detail_cm['origin_program'], request.detail_cm['origin_institution'])).font.bold = True
