...
class HCEMPRE():
    ...
    def case_HOMOLOGACION_CONVALIDACION_EQUIVALENCIA_PREGRADO(request, docx):
        ...
        para.add_run(' homologar en el periodo académico ' + request.academic_period)
        para.add_run(', el requisito de idioma inglés por obtener una calificación de ')
        para.add_run(request.detail_cm['min_grade'] + ' en el examen ' + request.detail_cm['institution'])
        para.add_run(', siendo ' + request.detail_cm['grade_got']+' el mínimo exigido.')
        ...
        cellp.add_run(request.student_name + '\t\tDNI. ' + request.student_dni).font.bold = True
        
        cellp = table.cell(1, 0).merge(table.cell(1, 4)).paragraphs[0]
        cellp.add_run('Asignaturas a homologar en el plan de estudios ' + 'Ing. Sistemas')
        ...
        cellp = table.cell(3, 5).merge(table.cell(len(request.detail_cm['subjects'])+3, 5)).paragraphs[0]
        cellp.add_run(request.detail_cm['institution'])

        cellp = table.cell(3, 6).merge(table.cell(len(request.detail_cm['subjects'])+3, 6)).paragraphs[0]
