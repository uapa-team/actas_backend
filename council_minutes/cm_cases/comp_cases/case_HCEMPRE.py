...
class HCEMPRE():
    ...
    def case_HOMOLOGACION_CONVALIDACION_EQUIVALENCIA_PREGRADO_NA(request, docx, paragraph):
        if 'homologation' in request.detail_cm:
            ...
            Item = Item1 + Item2
            para = docx.add_paragraph(Item.format(request.academic_period, request.detail_cm['homologation'][-1], request.detail_cm['homologation'][-2], request.justification), style='List Number')
            para.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
            para.paragraph_format.space_after = Pt(0)
            HCEMPRE.case_HOMOLOGACION_CONVALIDACION_EQUIVALENCIA_PREGRADO_TABLE_NA(request, docx,'homologation')
        if 'equivalence' in request.detail_cm:
            Item1 = 'Equivaler en el periodo académico {}, la(s) siguiente(s) asignatura(s)'
            Item2 = ' cursada(s) en el programa {} de la {}, debido a que {}. (Artículo 35 del Acuerdo 008 del 2008 del Consejo Superior Universitario)'                
            Item = Item1 + Item2
            para = docx.add_paragraph(Item.format( request.academic_period, request.detail_cm['equivalence'][-1], request.detail_cm['equivalence'][-2], request.justification), style='List Number')
            para.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
            para.paragraph_format.space_after = Pt(0)
            HCEMPRE.case_HOMOLOGACION_CONVALIDACION_EQUIVALENCIA_PREGRADO_TABLE_NA(request, docx,'equivalence')
        if 'recognition' in request.detail_cm:
            Item1 = 'Convalidar en el periodo académico {}, la(s) siguiente(s) asignatura(s)'
            Item2 = ' cursada(s) en el programa {} de la {}, debido a que {}.'                
            Item = Item1 + Item2
            para = docx.add_paragraph(Item.format( request.academic_period, request.detail_cm['recognition'][-1], request.detail_cm['recognition'][-2], request.justification), style='List Number')
            para.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
            para.paragraph_format.space_after = Pt(0)
            HCEMPRE.case_HOMOLOGACION_CONVALIDACION_EQUIVALENCIA_PREGRADO_TABLE_NA(request, docx,'recognition')
    ...
    def case_HOMOLOGACION_CONVALIDACION_EQUIVALENCIA_PREGRADO_TABLE(request, docx, case):
        ...
        for i in range (0, len(request.detail_cm[case]) - 2):           
            if len(request.detail_cm[case][i]['origin_subject']) > len(request.detail_cm[case][i]['subject_out']):
                ...
                credits = num_credits[request.detail_cm[case][i]['subject_out'][0]['tipology']]
                credits += int(request.detail_cm[case][i]['subject_out'][0]['credits'])
                num_credits[request.detail_cm[case][i]['subject_out'][0]['tipology']] = credits
                ...
                for j in range(0, len(request.detail_cm[case][i]['subject_out'])):
                    ...
                    credits = num_credits[request.detail_cm[case][i]['subject_out'][j]['tipology']]
                    credits += int(request.detail_cm[case][i]['subject_out'][j]['credits'])
                    num_credits[request.detail_cm[case][i]['subject_out'][j]['tipology']] = credits
                    num_credits['total'] += credits
                    row_a += 1
        cant_rows = 0
        appear = []
        ...
        for i in range(0, cant_rows):  
            cellp = table.cell(i + row_a, 0).merge(table.cell(i + row_a, 1)).paragraphs[0]
            cellp.add_run('Créditos homologados ' + appear[i-1] + ':')
            table.cell(i + row_a, 2).paragraphs[0].add_run(str(num_credits[appear[i-1]]))
        
        cellp = table.cell(row_a - 1, 3).merge(table.cell(cant_rows - 1 + row_a, 6)).paragraphs[0]        
    ...
    def case_HOMOLOGACION_CONVALIDACION_EQUIVALENCIA_PREGRADO_TABLE_NA(request, docx, case):
        case_d = {'homologation': 'homologan', 'equivalence': 'equivalen', 'recognition': 'convalidan' }
        cant_rows = 0
        ...
        cellp = table.cell(1, 0).merge(table.cell(1, 4)).paragraphs[0]
        cellp.alignment = WD_ALIGN_PARAGRAPH.CENTER
        cellp.add_run('Asignaturas que no se {} en el plan de estudios de {}({})'.format(case_d[case], request.get_academic_program_display(), request.academic_program)).font.bold = True
        table.cell(2, 0).paragraphs[0].add_run('Asignatura Universidad Nacional de Colombia ({})'.format(request.academic_program)).font.bold = True
        table.cell(2, 1).paragraphs[0].add_run('Asignatura ({})'.format(request.detail_cm[case][-2])).font.bold = True
        table.cell(2, 2).paragraphs[0].add_run('Justificación').font.bold = True
        ...
        for i in range (0, len(request.detail_cm[case]) - 3):           
            if len(request.detail_cm[case][i]['origin_subject']) > len(request.detail_cm[case][i]['subject_out']):
                ...
                cellp.add_run(request.detail_cm[case][i]['subject_out'][0]['grade'])
                for j in range(0, len(request.detail_cm[case][i]['origin_subject'])):
                    table.cell(row_a, 1).paragraphs[0].add_run(request.detail_cm[case][i]['origin_subject'][j]['name'])
                    table.cell(row_a, 2).paragraphs[0].add_run(request.detail_cm[case][i]['origin_subject'][j]['justification'])
                    row_a += 1
            else:
                row_m = row_a + len(request.detail_cm[case][i]['subject_out']) - 1
                cellp = table.cell(row_a, 1).merge(table.cell(row_m, 1)).paragraphs[0]
                cellp.add_run(request.detail_cm[case][i]['origin_subject'][0]['name'])
                cellp = table.cell(row_a, 2).merge(table.cell(row_m, 2)).paragraphs[0]
                cellp.add_run(request.detail_cm[case][i]['origin_subject'][0]['justification'])
                for j in range(0, len(request.detail_cm[case][i]['subject_out'])):
                    table.cell(row_a, 0).paragraphs[0].add_run(request.detail_cm[case][i]['subject_out'][j]['name'])
                    table.cell(row_a, 3).paragraphs[0].add_run(request.detail_cm[case][i]['subject_out'][j]['credits'])
                    table.cell(row_a, 4).paragraphs[0].add_run(request.detail_cm[case][i]['subject_out'][j]['grade'])
                    row_a += 1
