...
class TRASPOS():
    ...
    @staticmethod
    def case_TRASLADO_POSGRADO(request, docx):
        large_program = ''
        ...
        if request.approval_status == 'AP':
            ...
            para.add_run(' t')   
        para.add_run('raslado ' + request.detail_cm['tras_type'] + ' del programa de ' + request.detail_cm['origin'] + ' del plan de estudios de ' )
        para.add_run(request.detail_cm['or_inv_prof'] + ' de la Universidad Nacional de Colombia - Sede ' + request.detail_cm['campus_origin'])
        para.add_run(', al programa ' + large_program + ' del plan de estudios de ' +request.detail_cm['des_inv_prof'] + ' de la Universidad Nacional de Colombia - Sede Bogotá, debido a que ' + request.justification + '.')
        if request.approval_status == 'AP':
            para = docx.add_paragraph()
            para.alignment=WD_ALIGN_PARAGRAPH.JUSTIFY
            para.add_run('2. Equivaler en el programa '+ large_program + ', plan de estudios de ' +request.detail_cm['des_inv_prof'] +', ')
            para.add_run('en el periodo académico ' +request.detail_cm['period'] + ', las asignaturas cursadas en el programa ' + request.detail_cm['origin'] + '.')
            table = docx.add_table(rows=len(request.detail_cm['equivalencia'])+2, cols=6)
            ...
            table.cell(0, 0).paragraphs[0].add_run('ASIGNATURA QUE SE EQUIVALE').font.bold = True
